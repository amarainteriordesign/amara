...
export default function Studio({ isDesktopSideBySide = false }: StudioProps) {
  return (
    <section className={`w-full bg-[#000] ${isDesktopSideBySide ? "h-full flex items-center" : ""}`}>
      <div className={`mx-auto px-[40px] pt-[100px] pb-[80px] max-sm:px-[17px] max-sm:pt-[80px] max-sm:pb-[60px] ${isDesktopSideBySide ? "lg:max-w-full lg:px-[60px] lg:pt-[60px] lg:pb-[60px]" : "max-w-[800px]"}`}>
        <Image
          src="/images/pages/home/drawing.png"
          alt="Drawing"
          width={483}
          height={245}
          className={`mx-auto h-auto w-full object-cover pb-[55px] max-sm:pb-[60px] ${isDesktopSideBySide ? "lg:max-w-[400px] lg:pb-[40px]" : "max-w-[500px]"}`}
        />
        <div className="text-center">
          <p className={`font-display text-normal mx-auto pb-[18px] tracking-[0.8px] text-[#FFF] max-sm:max-w-full max-sm:pb-[28px] max-sm:text-[22px] max-sm:leading-[23px] max-sm:tracking-[0.4] ${isDesktopSideBySide ? "lg:max-w-full lg:text-[32px] lg:leading-[38px] lg:pb-[14px]" : "max-w-[500px] text-[42px] leading-[49px]"}`}>
          ...
          <p className={`text-normal mx-auto pb-[54px] font-serif text-[16px] leading-[24px] tracking-[-0.4] text-[#FFF] max-sm:max-w-full max-sm:pb-[35px] ${isDesktopSideBySide ? "lg:max-w-full lg:text-[14px] lg:leading-[22px] lg:pb-[30px]" : "max-w-[600px]"}`}>
        ...
        <p className={`mt-[50px] text-center font-sans text-[14px] leading-[30px] font-normal tracking-[1.8] text-[#FFF] uppercase max-sm:hidden ${isDesktopSideBySide ? "lg:mt-[30px]" : ""}`}>
          MIAMI - DUBAI - PARIS
        </p>
      </div>
    </section>
